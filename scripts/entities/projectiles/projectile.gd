	...
	if body is Unit and body.has_method("get_side"):
		...
		if is_valid_target:
			if is_michael_glaive:
				#print("Explosion du Glaive de Michaël !")
...
func _explode_area_damage(explosion_pos: Vector2) -> void:
	...
	query.collision_mask = 2 # Layer des unites
...
func _explode_michael_glaive(explosion_pos: Vector2) -> void:
	...
	# trouver toutes les unites dans la zone
	var space = get_world_2d().direct_space_state
	var query = PhysicsShapeQueryParameters2D.new()
	var shape = CircleShape2D.new()
	shape.radius = area_radius
	...
	for hit in hits:
		...
		if unit is Unit and unit.has_method("get_side"):
			# verifier si c'est une cible valide
			var is_valid_target = (targets_enfer and unit.get_side() == true) or (not targets_enfer and unit.get_side() == false)
			
			if is_valid_target:
				# calculer les dégats selon la taille de l'adversaire
				...
				#print("Glaive → ", unit.name, " (", unit.unit_size, "): ", damage_value, " dmg (", old_health, " → ", unit.get_health(), ")")
	
	#print("Total unités touchés: ", damaged_count)
